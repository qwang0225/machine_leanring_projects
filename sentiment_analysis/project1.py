...
def hinge_loss_single(feature_vector, label, theta, theta_0):
    ...
    """
    # Your code here
    prediction = theta.dot(feature_vector) + theta_0
...
def perceptron(feature_matrix, labels, T):
    ...
    # count = 0
    for t in range(T):
        for i in get_order(len(feature_matrix)):
            # prev_theta = theta
...
def average_perceptron(feature_matrix, labels, T):
    ...
    return theta_sum / (T * len(feature_matrix)), theta_0_sum / (T * len(feature_matrix))
...
def pegasos(feature_matrix, labels, T, L):
    ...
    """
    # Your code here
    # number of times of updates
    t = 1
    theta = np.zeros(feature_matrix[0].shape)
    theta_0 = 0
    for i in range(T):
        for j in get_order(len(feature_matrix)):
            theta_prev = np.copy(theta)
            theta_0_prev = theta_0

            theta, theta_0 = pegasos_single_step_update(feature_matrix[j], labels[j], L, 1 / np.sqrt(t), theta, theta_0)
...
def classify(feature_matrix, theta, theta_0):
    ...
    """
    # Your code here
    ret = np.zeros(feature_matrix.shape[0])
    for i in range(feature_matrix.shape[0]):
        result = theta.dot(feature_matrix[i]) + theta_0
        if result < 0 or np.abs(result) < 1e-10:
            ret[i] = -1
        else:
            ret[i] = 1
